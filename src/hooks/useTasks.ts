...
export function useTasks(): UseTasksState {
  ...
  function normalizeTasks(input: any[]): Task[] {
    ...
    return (Array.isArray(input) ? input : []).map((t, idx) => {
      ...
      // ensure unique ids
      const uniqueId = seenIds.has(safeId) ? `${safeId}-${idx}` : safeId;
      seenIds.add(uniqueId);
      return {
        id: uniqueId,
        title: String(t.title ?? '').trim() || `Untitled ${idx + 1}`,
        revenue: Number.isFinite(Number(t.revenue)) && Number(t.revenue) >= 0 ? Number(t.revenue) : 0,
  ...
  useEffect(() => {
    if (fetchedRef.current) return; // guard against StrictMode double effect
    fetchedRef.current = true;
    let isMounted = true;
    async function load() {
      try {
        // Abort fetch if it takes too long (network hang protection)
        ...
        // On error or timeout, fall back to generated data so UI stays usable
        if (isMounted) {
          setError(e?.name === 'AbortError' ? 'Network timeout — showing sample data' : (e?.message ?? 'Failed to load tasks'));
  ...
  return { tasks, loading, error, derivedSorted, metrics, lastDeleted, addTask, updateTask, deleteTask, undoDelete, dismissUndo };
}
